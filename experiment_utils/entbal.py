...
def eb_weights(pars, XD, bw):
    """Calculates weights from optimized parameters."""
    XDP = -XD.dot(pars)
    # Numerical stability trick
    max_XDP = np.max(XDP)
    Q = bw * np.exp(XDP - max_XDP)
    # Normalize weights for the subgroup
    return Q / np.sum(Q)
...
class entbal:
    ...
    def fit(self, X, TA, estimand="ATE", n_moments=2, base_weights=None):
        ...
        elif estimand.upper() == "ATC":
            targets = np.mean(XD[TA_np == 0], axis=0)
        else:
            raise ValueError("Invalid Estimand. Choose from 'ATE', 'ATT', 'ATC'.")

        self.W = np.copy(base_weights)
        control_mask = TA_np == 0
        treated_mask = TA_np == 1
        initial_params = np.zeros(XD.shape[1])

        optimizer_args = {"method": "L-BFGS-B"}

        if estimand.upper() in ["ATE", "ATT"]:
            res0 = minimize(
                eb_loss, initial_params, args=(XD[control_mask], base_weights[control_mask], targets), **optimizer_args
            )
            self.W[control_mask] = eb_weights(res0.x, XD[control_mask], base_weights[control_mask])

        if estimand.upper() in ["ATE", "ATC"]:
            res1 = minimize(
                eb_loss, initial_params, args=(XD[treated_mask], base_weights[treated_mask], targets), **optimizer_args
            )
            self.W[treated_mask] = eb_weights(res1.x, XD[treated_mask], base_weights[treated_mask])
