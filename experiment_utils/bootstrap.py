...
class BootstrapMixin:
    ...
    def _bootstrap_single_iteration(
        ...
        """
        try:
            # Use pre-generated indices if available (optimization for multiple outcomes/models)
            if bootstrap_indices is not None:
                boot_data = data.iloc[bootstrap_indices].reset_index(drop=True)
            else:
                # Fallback to on-the-fly resampling (backward compatibility)
    ...
    def _bootstrap_single_effect(
        ...
        if use_parallel:
            ...
            # Use configured backend (default: threading for memory efficiency)
            # threading: shares memory, avoids expensive serialization (recommended for large DataFrames)
