...
class ExperimentAnalyzer:
    ...
    def __init__(
        ...
        plot_common_support: bool = False,
    ) -> None:
        ...
        self._plot_common_support = plot_common_support
        ...
        for experiment_tuple, temp_pd in grouped_data:
            ...
            if len(final_covariates) > 0:
                ...
                if adjustment == "IPW":
                    ...
                    if self._plot_common_support:
                        # Ensure propensity scores exist before plotting
                        if "propensity_score" in temp_pd.columns:
                            self._create_plot_common_support(
                                df=temp_pd,
                            ...
                            self._logger.warning("Propensity score column not found, skipping common support plot.")
    ...
    def __transform_tuple_column(self, df: pd.DataFrame, tuple_column: str, new_columns: list[str]) -> pd.DataFrame:
        ...
        if len(new_columns) == 1:
            ...
            if not df.empty and df[tuple_column].apply(check_tuple).all():
                ...
                self._logger.warning(
                    f"Column '{tuple_column}' does not contain consistent tuples of length "
                    f"{len(new_columns)}. Transformation skipped."
                )
        return df
        ...
        if self._exp_sample_ratio_col is not None:
            ...
            try:
                ...
                if p_value < self._alpha:
                    srm_detected = True
                    self._logger.info(
                        f"Significant mismatch detected (p < {self._alpha:.3f}). "
                        "Observed ratio differs statistically from expected ratio."
                    )
    ...
    def _create_plot_common_support(
        ...
        """
        # Get propensity scores for each group (drop NaN values)
        ...
        # Define a common grid for density evaluation based on the combined data
        all_scores = df[propensity_col].dropna()
        x_min, x_max = all_scores.min(), all_scores.max()
        x_grid = np.linspace(x_min, x_max, grid_size)

        # Estimate densities for each group
        ...
        # Adjust y-tick labels to display as absolute values
