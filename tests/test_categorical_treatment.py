...
def sample_data_categorical():
    ...
    # Treatment effects
    effects = {"control": 0.0, "treatment_a": 0.05, "treatment_b": 0.10}
...
def sample_data_numeric_categorical():
    ...
    data = pd.DataFrame(
        {"experiment_id": 1, "treatment": groups, "conversion": conversion, "baseline_conversion": baseline_conversion}
    )
...
def test_categorical_treatment_specific_comparisons(sample_data_categorical):
    ...
    comparisons = [("treatment_a", "control"), ("treatment_b", "control")]
    ...
    comparison_tuples = set(zip(results["treatment_group"], results["control_group"], strict=False))
...
def test_binary_treatment_default_comparison():
    ...
    data = pd.DataFrame({"experiment_id": 1, "treatment": treatment, "conversion": conversion, "baseline": baseline})

    analyzer = ExperimentAnalyzer(
        data=data, outcomes="conversion", treatment_col="treatment", experiment_identifier="experiment_id"
    )
...
def test_multiple_outcomes_categorical(sample_data_categorical):
    ...
    sample_data_categorical["revenue"] = sample_data_categorical["conversion"] * np.random.gamma(
        2, 10, len(sample_data_categorical)
    )
...
def test_sample_ratio_with_categorical():
    ...
    if "srm_detected" in results.columns:
        assert results["srm_detected"].iloc[0] or results["srm_detected"].iloc[0] == 1
