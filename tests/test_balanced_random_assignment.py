...
def test_same_seed_same_result(base_df):
    a1 = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.5)
    a2 = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.5)
    pd.testing.assert_series_equal(a1, a2)


def test_different_seeds_differ(base_df):
    a1 = balanced_random_assignment(base_df, seed=1, allocation_ratio=0.5)
    a2 = balanced_random_assignment(base_df, seed=2, allocation_ratio=0.5)
...
def test_binary_exact_50_50(base_df):
    a = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.5)
...
def test_binary_exact_70_30(base_df):
    a = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.7)
    counts = a.value_counts()
...
def test_three_variants_equal_allocation(base_df):
    a = balanced_random_assignment(base_df, seed=42, variants=["control", "variant_a", "variant_b"])
    counts = a.value_counts()
...
def test_non_default_index(base_df):
    ...
    a = balanced_random_assignment(df_shifted, seed=42, allocation_ratio=0.5)
...
def test_non_sequential_index():
    ...
    a = balanced_random_assignment(df, seed=42, allocation_ratio=0.5)
...
def test_stratification_different_from_unstratified(base_df):
    ...
    a_strat = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.5, stratification_covariates=["segment"])
    a_unstrat = balanced_random_assignment(base_df, seed=42, allocation_ratio=0.5)
...
def test_binary_integer_labels_50_50(base_df):
    ...
    a = balanced_random_assignment(base_df, seed=42, variants=[0, 1], allocation_ratio={0: 0.5, 1: 0.5})
...
def test_binary_integer_labels_allocation_ratio_float(base_df):
    ...
    a = balanced_random_assignment(base_df, seed=42, variants=[0, 1], allocation_ratio={0: 0.3, 1: 0.7})
    counts = a.value_counts()
    assert counts[0] == 300
    assert counts[1] == 700
...
def test_integer_labels_reproducible(base_df):
    a1 = balanced_random_assignment(base_df, seed=7, variants=[0, 1], allocation_ratio={0: 0.5, 1: 0.5})
    a2 = balanced_random_assignment(base_df, seed=7, variants=[0, 1], allocation_ratio={0: 0.5, 1: 0.5})
    pd.testing.assert_series_equal(a1, a2)
...
def test_float_ratio_with_integer_variants(base_df):
    ...
    a = balanced_random_assignment(base_df, seed=42, variants=[1, 0], allocation_ratio=0.667)
    counts = a.value_counts()
...
def test_float_ratio_with_string_variants(base_df):
    ...
    a = balanced_random_assignment(base_df, seed=42, variants=["treatment", "control"], allocation_ratio=0.6)
    counts = a.value_counts()
...
def test_float_ratio_variants_reversed_label_order(base_df):
    ...
    a_1_first = balanced_random_assignment(base_df, seed=42, variants=[1, 0], allocation_ratio=0.7)
    a_0_first = balanced_random_assignment(base_df, seed=42, variants=[0, 1], allocation_ratio=0.7)
...
def test_allocation_ratio_dict_without_variants_raises():
    ...
    with pytest.raises(ValueError, match="allocation_ratio must be a float"):
        balanced_random_assignment(df, allocation_ratio={"a": 0.5, "b": 0.5})
